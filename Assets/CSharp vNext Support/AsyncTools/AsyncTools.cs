...
public static class AsyncTools
{
	...
	public static void WhereAmI(string text)
	{
		if (IsMainThread())
		{
			var contextName = (System.Threading.SynchronizationContext.Current as UnitySynchronizationContext)?.Name ?? "No context";
			Debug.Log($"{text}: main thread, {contextName}, frame: {Time.frameCount}");
		}
		else
		{
			Debug.Log($"{text}: background thread, id: {System.Threading.Thread.CurrentThread.ManagedThreadId}");
		}
	}
	...
	/// </summary>
	public static bool IsMainThread() => System.Threading.Thread.CurrentThread.ManagedThreadId == UnityScheduler.MainThreadId;
	...
	public static Task<byte[]> DownloadAsBytesAsync(string address, System.Threading.CancellationToken cancellationToken = new System.Threading.CancellationToken())
	{
	...
	/// <param name="address">File URL</param>
	/// <param name="cancellationToken">Optional cancellation token</param>
	public static Task<string> DownloadAsStringAsync(string address, System.Threading.CancellationToken cancellationToken = new System.Threading.CancellationToken())
	{
		return Task.Factory.StartNew(
			delegate
			{
	...
	{
		var context = System.Threading.SynchronizationContext.Current as UnitySynchronizationContext;
	...
	private class SynchronizationContextAwaiter : Awaiter
	{
		...
		public override bool IsCompleted => context == null || context == System.Threading.SynchronizationContext.Current;
	...
	private class ThreadPoolContextAwaiter : Awaiter
	{
		...
		public override void OnCompleted(Action action) => System.Threading.ThreadPool.QueueUserWorkItem(state => action(), null);
	}
